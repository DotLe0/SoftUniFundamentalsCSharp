...
namespace _11.ArrayManipulator
{
    internal class Program
    {
        ...
        private static void LastElementsForCondition(int[] array, int count, bool isEven)
        {
            ...
            {
                int[] result = array.Where(c => isEven == (c % 2 == 0)).TakeLast(count).ToArray();
                Console.WriteLine("[{0}]", string.Join(", ", result));
            }
        }

        private static void FirstElementsForCondition(int[] array, int count, bool isEven)
        {
            if (count > array.Length)
            {
                Console.WriteLine("Invalid count");
            ...
            {
                int[] result = array.Where(c => isEven == (c % 2 == 0)).Take(count).ToArray();
                Console.WriteLine("[{0}]", string.Join(", ", result));
            }
        }
        ...
        {
            if (!array.Any(c => isEven == (c % 2 == 0)))
            {
                Console.WriteLine("No matches");
            }
            else
            {
                int min = array.Where(c => isEven == (c % 2 == 0)).Min();
        ...
        private static void MaxIndexForCondition(int[] array, bool isEven)
        {
            ...
            {
                int max = array.Where(c => isEven == (c % 2 == 0)).Max();
